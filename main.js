...
function queryTickets(config) {
	...
	var req = https.get(options, function (res) {
		...
		var transporter = nodemailer.createTransport({
			host: "smtp.qq.com",//邮箱的服务器地址，如果你要换其他类型邮箱（如QQ）的话，你要去找他们对应的服务器，
		...
		res.on('end', function () {
			...
			for (var j = 0; j < train_arr.length; j++) {
				...
				console.log('\n ' + trainNum + ' 车次的硬卧余票数:', yw, ', 软卧余票数:', rw, '。当前时间：' + getTime());
				if (yw != '无' && yw != '--' || rw != '无' && rw != '--') {
					if (yw_temp[j] == yw && rw_temp[j] == rw) {//当余票状态发生改变的时候就不发送邮件
					...
					var mailOptions = {
						...
						subject: trainNum + '有票啦，硬卧：' + yw + '，软卧：' + rw, // 邮件标题
